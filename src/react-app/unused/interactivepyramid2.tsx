import { useState, useEffect, useMemo, FC, ReactNode, useRef } from 'react';
 ...
 * through an animated "Neural Assembler" visualization with a scroll-driven central glow.
 */
export default function InteractivePyramidSection() {
  ...
  // Observer for activating icon and text animations based on which text is in view
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) setActiveStep(-1);
    };
  ...
  // Effect for tracking overall scroll progress to control the central logo's glow
  useEffect(() => {
    const handleScroll = () => {
  ...
  const coreGlowStyle = {
    filter: `drop-shadow(0 0 ${scrollProgress * 60}px hsla(180, 100%, 50%, 0.8)) ` + 
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <div className="sticky top-24 h-[500px] lg:h-[600px]" aria-hidden="true">
            <div className="relative w-full h-full">
              ...
              <div
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 flex items-center justify-center"
                style={coreGlowStyle}
              >
                <img 
